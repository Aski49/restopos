...
$navItems = [
    ...
    ['page'=>'menu_qr',    'icon'=>'📱',   'label'=>'Menu QR Code'],
    ['page'=>'online_orders','icon'=>'🛒', 'label'=>'Online Orders'],
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<?= $nav['page'] ?>.php"
           ...
           id="nav-<?= $nav['page'] ?>">
            ...
            <?php if ($nav['page']==='online_orders' && canAccess('online_orders')): ?>
              <?php
                $newOnlineOrders = 0;
                try {
                    $stmt = getDB()->query("SELECT COUNT(*) FROM online_orders WHERE seen=0 AND status='new'");
                    $newOnlineOrders = (int)$stmt->fetchColumn();
                } catch(Exception $e){}
              ?>
              <?php if ($newOnlineOrders > 0): ?>
                <span id="onlineOrderBadge" style="background:var(--red);color:#fff;border-radius:100px;padding:1px 7px;font-size:10px;font-weight:700;margin-left:auto"><?=$newOnlineOrders?></span>
              <?php else: ?>
                <span id="onlineOrderBadge" style="display:none;background:var(--red);color:#fff;border-radius:100px;padding:1px 7px;font-size:10px;font-weight:700;margin-left:auto"></span>
              <?php endif; ?>
            <?php endif; ?>
        </a>
        <?php endforeach; ?>
