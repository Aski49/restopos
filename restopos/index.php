    ...
    if ($username && $password) {
        try {
            ...
            if ($user && $user['password'] === $password) {
                ...
                logActivity('Logged In', 'auth', $user['name'].' ('.$user['role'].') signed in');

                // Send each role straight to the first page they're allowed to see
                $allowed  = ROLE_PAGES[$user['role']] ?? ROLE_PAGES['cashier'];
                $homePage = $allowed[0] ?? 'dashboard';
