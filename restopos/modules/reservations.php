...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_res') {
        $db->prepare("INSERT INTO reservations(customer_name,contact,res_date,res_time,pax,location,notes,status,created_by)
                      VALUES(?,?,?,?,?,?,?,?,?)")
           ->execute([
        ...
        logActivity('Created Reservation', 'reservations', 'Reservation for '.trim($_POST['customer_name']).' on '.$_POST['res_date'].' '.$_POST['res_time']);
        $msg = ['type'=>'success','text'=>'Reservation added.'];

    } elseif ($action === 'edit_res') {
        $db->prepare("UPDATE reservations SET customer_name=?,contact=?,res_date=?,res_time=?,pax=?,location=?,notes=?,status=? WHERE id=?")
           ->execute([
               trim($_POST['customer_name']),
               trim($_POST['contact']),
               $_POST['res_date'],
...
<div class="card">
  ...
  <div class="table-wrap">
    <table class="data-table" id="resTable">
      <thead>
        <tr><th>Name</th><th>Contact</th><th>Date</th><th>Time</th><th>Pax</th><th>Location</th><th>Notes</th><th>Status</th><th>Actions</th></tr>
      </thead>
      <tbody>
      <?php foreach ($reservations as $r): ?>
        <tr class="res-row" data-search="<?=strtolower(htmlspecialchars($r['customer_name'].' '.$r['contact'].' '.$r['location']))?>">
          ...
          <td class="mono text-accent fw-700"><?=date('h:i A',strtotime($r['res_time']))?></td>
          <td class="text-right mono"><?=$r['pax']?></td>
          <td class="text-muted fs-12"><?=htmlspecialchars($r['location']?:'—')?></td>
          <td class="text-muted fs-12" style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="<?=htmlspecialchars($r['notes'])?>"><?=htmlspecialchars($r['notes']?:'—')?></td>
          <td>
  ...
  <div class="modal-box">
    ...
    <form method="POST">
      ...
      <div class="form-row form-row-2 mb-12">
        ...
        <div class="form-group"><label class="form-label">Time *</label><input type="time" name="res_time" id="resTime" class="form-control" required></div>
        ...
        <div class="form-group"><label class="form-label">Location / Table</label><input name="location" id="resLocation" class="form-control" placeholder="e.g. Table T5, Garden Area"></div>
        <div class="form-group" style="grid-column:1/-1"><label class="form-label">Notes</label><textarea name="notes" id="resNotes" class="form-control" rows="2" placeholder="Special requests, allergies, occasion..."></textarea></div>
