          ...
          <td>
            <div style="display:flex;gap:5px;flex-wrap:wrap">
              ...
              <button class="btn btn-sm btn-outline"
                ...
                data-phone="<?=htmlspecialchars($d['phone']??'',ENT_QUOTES)?>"
                data-email="<?=htmlspecialchars($d['email']??'',ENT_QUOTES)?>"
                data-limit="<?=$d['credit_limit']?>"
                onclick="openEditDebtor(this)">✏</button>
...
function openEditDebtor(btn) {
  document.getElementById('debtorModalTitle').textContent = 'Edit Debtor';
  document.getElementById('debtorAction').value   = 'edit_debtor';
  document.getElementById('editDebtorId').value   = btn.dataset.id;
  document.getElementById('debtorName').value     = btn.dataset.name;
  document.getElementById('debtorPhone').value    = btn.dataset.phone || '';
  document.getElementById('debtorEmail').value    = btn.dataset.email || '';
  document.getElementById('debtorLimit').value    = btn.dataset.limit || 0;
