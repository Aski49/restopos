...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ── Add user ──────────────────────────────────────────────
        ...
        $validRoles = ['admin','manager','cashier','kitchen'];
        if (!in_array($role, $validRoles)) {
            $msg = ['type'=>'danger','text'=>'Please select a valid role.'];
        } else {
            ...
            if ($exists->fetch()) {
                ...
                $db->prepare("INSERT INTO users(name,username,password,role) VALUES(?,?,?,?)")
                   ->execute([trim($_POST['name']),$uname,trim($_POST['password']),$role]);
        ...
        if (!in_array($role, $validRoles)) {
            ...
            if ($pass) {
                $db->prepare("UPDATE users SET name=?,username=?,password=?,role=? WHERE id=?")
                   ->execute([trim($_POST['name']),trim($_POST['username']),$pass,$role,$uid]);
            } else {
                $db->prepare("UPDATE users SET name=?,username=?,role=? WHERE id=?")
                   ->execute([trim($_POST['name']),trim($_POST['username']),$role,$uid]);
            }
      ...
      <table class="data-table">
        ...
        ?>
          <tr>
            ...
            <td>
              <span class="badge <?=$u['role']==='admin'?'badge-red':($u['role']==='manager'?'badge-accent':($u['role']==='kitchen'?'badge-blue':'badge-muted'))?>">
                <?=$u['role']?ucfirst($u['role']):'⚠ No Role Set'?>
              </span>
            </td>
            <td>
      ...
      <div class="form-row form-row-2 mb-12">
        ...
        <div class="form-group">
          ...
          <select name="role" id="uRole" class="form-control" required>
            ...
            <option value="kitchen">Kitchen Boy</option>
...
function openEditUser(btn) {
  document.getElementById('userModalTitle').textContent = 'Edit User — ' + btn.dataset.username;
  ...
  document.getElementById('uPassword').value    = btn.dataset.password;
