...
if($_SERVER['REQUEST_METHOD']==='POST'){
    $action=$_POST['action']??'';
    if($action==='add'){
        $db->prepare("INSERT INTO menu_items(category_id,name,price,description) VALUES(?,?,?,?)")->execute([$_POST['category_id'],$_POST['name'],$_POST['price'],$_POST['description']??'']);
        $msg=['type'=>'success','text'=>'Menu item added.'];
    } elseif($action==='edit'){
        $db->prepare("UPDATE menu_items SET category_id=?,name=?,price=?,description=? WHERE id=?")->execute([$_POST['category_id'],$_POST['name'],$_POST['price'],$_POST['description']??'',$_POST['item_id']]);
        $msg=['type'=>'success','text'=>'Item updated.'];
    } elseif($action==='delete_item'){
        $db->prepare("DELETE FROM menu_items WHERE id=?")->execute([$_POST['item_id']]);
        $msg=['type'=>'success','text'=>'Menu item deleted.'];
    } elseif($action==='toggle'){
        $db->prepare("UPDATE menu_items SET active=NOT active WHERE id=?")->execute([$_POST['item_id']]);
        $msg=['type'=>'success','text'=>'Item status changed.'];
...
<div class="card">
  ...
  <div class="table-wrap">
    <table class="data-table">
      <thead><tr><th>Item Name</th><th>Category</th><th class="text-right">Price</th><th>Description</th><th>Status</th><th>Actions</th></tr></thead>
      <tbody>
        ...
        <tr style="opacity:<?=$i['active']?1:.55?>">
          ...
          <td>
            <div style="display:flex;gap:6px">
              <button class="btn btn-sm btn-outline" onclick="editItem(<?=json_encode($i)?>)">Edit</button>
