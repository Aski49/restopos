...
$today = date('Y-m-d');
$todayCashSales = $db->prepare("SELECT COALESCE(SUM(total),0) as t FROM bills WHERE DATE(created_at)=? AND status='settled' AND payment_method='Cash'");
$todayCashSales->execute([$today]); $todayCashSales = $todayCashSales->fetch()['t'];

$todayCashExp = $db->prepare("SELECT COALESCE(SUM(amount),0) as t FROM expenses WHERE expense_date=? AND payment_method='Cash'");
$todayCashExp->execute([$today]); $todayCashExp = $todayCashExp->fetch()['t'];

$todayCard = $db->prepare("SELECT COALESCE(SUM(total),0) as t FROM bills WHERE DATE(created_at)=? AND status='settled' AND payment_method='Card'");
$todayCard->execute([$today]); $todayCard = $todayCard->fetch()['t'];

$todayPlat = $db->prepare("SELECT COALESCE(SUM(total),0) as t FROM bills WHERE DATE(created_at)=? AND status='settled' AND payment_method IN('Uber Eats','PickMe')");
$todayPlat->execute([$today]); $todayPlat = $todayPlat->fetch()['t'];
          ...
          <td>
            <div style="display:flex;gap:5px">
              <button class="btn btn-sm btn-outline"
                ...
                data-date="<?=$t['txn_date']?>"
                data-type="<?=htmlspecialchars($t['type'],ENT_QUOTES)?>"
                data-amount="<?=$t['amount']?>"
                data-desc="<?=htmlspecialchars($t['description']??'',ENT_QUOTES)?>"
                data-ref="<?=htmlspecialchars($t['reference']??'',ENT_QUOTES)?>"
                onclick="openEditTxn(this)">✏</button>
...
function openEditTxn(btn) {
  ...
  document.getElementById('txnId').value       = btn.dataset.id;
  document.getElementById('txnAccount').value  = btn.dataset.account;
  document.getElementById('txnDate').value     = btn.dataset.date;
  document.getElementById('txnType').value     = btn.dataset.type;
  document.getElementById('txnAmount').value   = btn.dataset.amount;
  document.getElementById('txnDesc').value     = btn.dataset.desc || '';
  document.getElementById('txnRef').value      = btn.dataset.ref  || '';
