...
// Cashier cannot view Payroll or Attendance report data — redirect to P&L if attempted
if (reportTabBlocked($rpt)) {
    header('Location: reports.php?rpt=pl&from='.urlencode($from).'&to='.urlencode($to));
    exit;
}
...
<!-- Tabs -->
<div class="tab-bar">
  <?php $tabs=['pl'=>'📊 P&L','expenses'=>'💸 Expenses','cashflow'=>'💵 Cash Flow','debtors'=>'🏦 Debtors','inventory'=>'📦 Inventory','payroll'=>'👥 Payroll','attendance'=>'📅 Attendance','promotions'=>'🎉 Promotions','online_orders'=>'🛒 Online Orders'];
  foreach ($tabs as $key=>$label):
    if (reportTabBlocked($key)) continue; // cashier cannot see Payroll / Attendance tabs
  ?>
  ...
  <div class="table-wrap">
    <table class="data-table">
      ...
      <?php foreach($attRows as $a):
        $otPay=round(100*$a['overtime_hours'],2);
      ?><tr>
...
<?php elseif ($rpt === 'online_orders'):
  $onlineOrders = $db->prepare("SELECT * FROM online_orders WHERE DATE(created_at) BETWEEN ? AND ? ORDER BY created_at DESC");
  $onlineOrders->execute([$from, $to]);
  $onlineOrders = $onlineOrders->fetchAll();
  $onlineTotals = ['revenue'=>0,'orders'=>count($onlineOrders),'completed'=>0,'cancelled'=>0];
  $byType = ['takeaway'=>['count'=>0,'total'=>0],'card'=>['count'=>0,'total'=>0],'bank_transfer'=>['count'=>0,'total'=>0]];
  foreach ($onlineOrders as $oo) {
    if ($oo['status'] !== 'cancelled') {
      $onlineTotals['revenue'] += $oo['total'];
    }
    if ($oo['status'] === 'completed') $onlineTotals['completed']++;
    if ($oo['status'] === 'cancelled') $onlineTotals['cancelled']++;
    if (isset($byType[$oo['order_type']])) {
      $byType[$oo['order_type']]['count']++;
      if ($oo['status'] !== 'cancelled') $byType[$oo['order_type']]['total'] += $oo['total'];
    }
  }
  $typeLabels = ['takeaway'=>'🥡 Takeaway','card'=>'💳 Card','bank_transfer'=>'🏦 Bank Transfer'];
  $statusColors = ['new'=>'badge-red','confirmed'=>'badge-blue','preparing'=>'badge-accent','ready'=>'badge-green','completed'=>'badge-muted','cancelled'=>'badge-red'];
?>
<div class="stats-grid mb-16">
  <div class="stat-card"><span class="stat-icon">🛒</span><div class="stat-label">Total Online Orders</div><div class="stat-value text-blue"><?=$onlineTotals['orders']?></div></div>
  <div class="stat-card"><span class="stat-icon">💰</span><div class="stat-label">Online Revenue</div><div class="stat-value text-green"><?=fmt($onlineTotals['revenue'])?></div></div>
  <div class="stat-card"><span class="stat-icon">✅</span><div class="stat-label">Completed</div><div class="stat-value text-accent"><?=$onlineTotals['completed']?></div></div>
  <div class="stat-card"><span class="stat-icon">❌</span><div class="stat-label">Cancelled</div><div class="stat-value text-red"><?=$onlineTotals['cancelled']?></div></div>
</div>
<div class="alert alert-info mb-16 fs-12">
  💡 <strong>Note:</strong> This report reads directly from the <strong>Online Orders</strong> table — it is independent of Bill History. Deleting a bill from Bill History does <strong>not</strong> remove it here. To remove an online order from this report, delete it from the <strong>Online Orders</strong> module.
</div>

<div class="card mb-16">
  <div class="card-title">Revenue by Order Type</div>
  <div class="table-wrap">
    <table class="data-table">
      <thead><tr><th>Order Type</th><th class="text-right">Orders</th><th class="text-right">Revenue</th></tr></thead>
      <tbody>
      <?php foreach ($byType as $type => $data): ?>
        <tr>
          <td class="fw-700"><?=$typeLabels[$type]??$type?></td>
          <td class="text-right mono text-blue"><?=$data['count']?></td>
          <td class="text-right mono text-green fw-700"><?=fmt($data['total'])?></td>
        </tr>
      <?php endforeach; ?>
      </tbody>
      <tfoot><tr><td><strong>TOTAL</strong></td><td class="text-right mono"><strong><?=$onlineTotals['orders']?></strong></td><td class="text-right mono text-green"><strong><?=fmt($onlineTotals['revenue'])?></strong></td></tr></tfoot>
    </table>
  </div>
</div>

<div class="card">
  <div class="card-title">All Online Orders — <?=$from?> → <?=$to?></div>
  <?php if (empty($onlineOrders)): ?>
    <div class="alert alert-info">No online orders recorded for this period.</div>
  <?php else: ?>
  <div class="table-wrap">
    <table class="data-table">
      <thead><tr><th>Order No</th><th>Customer</th><th>Phone</th><th>Date/Time</th><th>Type</th><th>Status</th><th class="text-right">Total</th><th>Note</th></tr></thead>
      <tbody>
      <?php foreach ($onlineOrders as $oo): ?>
        <tr>
          <td class="mono text-accent fw-700"><?=htmlspecialchars($oo['order_no'])?></td>
          <td class="fw-700"><?=htmlspecialchars($oo['customer_name'])?></td>
          <td class="mono text-muted fs-12"><?=htmlspecialchars($oo['customer_phone'])?></td>
          <td class="fs-12 text-muted mono"><?=date('d/m/Y H:i',strtotime($oo['created_at']))?></td>
          <td><?=$typeLabels[$oo['order_type']]??$oo['order_type']?></td>
          <td><span class="badge <?=$statusColors[$oo['status']]??'badge-muted'?>"><?=ucfirst($oo['status'])?></span></td>
          <td class="text-right mono fw-700" style="color:<?=$oo['status']==='cancelled'?'var(--red)':'var(--green)'?>"><?=fmt($oo['total'])?></td>
          <td class="fs-12 text-muted"><?=htmlspecialchars($oo['customer_note']?:'—')?></td>
        </tr>
      <?php endforeach; ?>
      </tbody>
    </table>
