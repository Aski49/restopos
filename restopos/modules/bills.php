...
$total = array_sum(array_column(array_filter($bills,fn($b)=>$b['status']==='settled'),'total'));
...
<div class="stats-grid mb-20">
  <div class="stat-card"><span class="stat-icon">🧾</span><div class="stat-label">Bills</div><div class="stat-value text-blue"><?=count($bills)?></div></div>
  ...
  <div class="stat-card"><span class="stat-icon">✅</span><div class="stat-label">Settled</div><div class="stat-value text-green"><?=count(array_filter($bills,fn($b)=>$b['status']==='settled'))?></div></div>
    ...
    <table class="data-table">
      ...
      <?php foreach ($bills as $b): ?>
        <tr>
          ...
          <td class="text-right mono <?=$b['status']==='voided'?'text-muted line-through':'text-green fw-700'?>"><?=fmt($b['total'])?></td>
          ...
          <td>
            <div style="display:flex;gap:5px;flex-wrap:wrap">
              <a href="print_bill.php?id=<?=$b['id']?>" target="_blank" class="btn btn-sm btn-outline">🖨 Print</a>
              <?php $bedit = ["id"=>$b["id"],"order_type"=>$b["order_type"],"table_no"=>$b["table_no"]??"","payment_method"=>$b["payment_method"],"notes"=>$b["notes"]??""]; ?>
              <button class="btn btn-sm btn-outline" onclick='openBillEdit(<?=json_encode($bedit)?>)'>
                ✏ Edit
              </button>
              ...
              <?php endif; ?>
              <form method="POST" style="display:inline" onsubmit="return confirm('Permanently DELETE bill <?=htmlspecialchars($b['bill_no'])?>? This cannot be undone.')">
      ...
      <tfoot><tr><td colspan="5"><strong>Total (Settled)</strong></td><td class="text-right mono text-green"><strong><?=fmt($total)?></strong></td><td colspan="3"></td></tr></tfoot>
    </table>
  </div>
</div>
...
function openBillEdit(b) {
  ...
  document.getElementById('editOrderType').value    = b.order_type;
  document.getElementById('editTableNo').value      = b.table_no || '';
  document.getElementById('editPayMethod').value    = b.payment_method;
  document.getElementById('editBillNotes').value    = b.notes || '';
