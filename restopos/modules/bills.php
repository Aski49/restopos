...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    if ($action === 'void') {
        $bn = $db->prepare("SELECT bill_no FROM bills WHERE id=?"); $bn->execute([$_POST['bill_id']]); $bn=$bn->fetchColumn();
        $db->prepare("UPDATE bills SET status='voided' WHERE id=?")->execute([$_POST['bill_id']]);
        logActivity('Voided Bill', 'bills', 'Bill '.$bn.' marked as voided');
        ...
        logActivity('Deleted Bill', 'bills', 'Bill '.$bn.' permanently deleted');
        ...
        logActivity('Edited Bill', 'bills', 'Bill ID '.$_POST['bill_id'].' details updated');
