...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['f_total'])) {
    ...
    if (!empty($cartData) && $total > 0) {
        try {
            ...
            // Recipe-based auto stock deduction is optional — only runs if the
            // menu_item_recipes / stock_usage tables exist (i.e. the inventory
            // usage migration has been applied). Older installs without the
            // migration continue to work exactly as before.
            $recipesEnabled = true;
            try {
                $db->query("SELECT 1 FROM menu_item_recipes LIMIT 1");
            } catch (Exception $e) {
                $recipesEnabled = false;
            }
            if ($recipesEnabled) {
                $recipeStmt = $db->prepare("SELECT inventory_item_id, qty_per_unit FROM menu_item_recipes WHERE menu_item_id=?");
                $deductStmt = $db->prepare("UPDATE inventory_items SET qty=GREATEST(0,qty-?) WHERE id=?");
                $usageStmt  = $db->prepare("INSERT INTO stock_usage(item_id,usage_date,qty,source,bill_id,menu_item_name) VALUES(?,?,?,?,?,?)");
                $today = date('Y-m-d');
            }
            ...
            foreach ($cartData as $item) {
                ...
                // ── Auto stock-out: deduct ingredients per recipe × qty sold ──
                if ($recipesEnabled && !empty($item['id'])) {
                    $recipeStmt->execute([$item['id']]);
                    $ingredients = $recipeStmt->fetchAll();
                    foreach ($ingredients as $ing) {
                        $usedQty = $ing['qty_per_unit'] * $item['qty'];
                        if ($usedQty > 0) {
                            $deductStmt->execute([$usedQty, $ing['inventory_item_id']]);
                            $usageStmt->execute([$ing['inventory_item_id'], $today, $usedQty, 'bill', $billId, $item['name']]);
                        }
                    }
                }
            }
