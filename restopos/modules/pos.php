...
// ── Override renderCart to include promo ──────────────────────
window.addEventListener('load', function() {
  // Override setPayMethod in app.js
  window.setPayMethod = posSetPay;

  // Patch renderCart to handle promo
  var _orig = window.renderCart;
  window.renderCart = function() {
    if (_orig) _orig();
    // Now apply promo discount on top
    var sub = cart.reduce(function(s,i){return s+i.price*i.qty;},0);
    var svc = (typeof orderType!=='undefined'&&orderType==='Dine-In') ? sub*(window.SVC_PCT||10)/100 : 0;
    var disc = sub*(typeof discountPct!=='undefined'?discountPct:0)/100;
    promoDiscAmt = calcPromoDisc(sub);
    ...
    // Recalculate total with promo
    var tax  = (sub + svc - disc - promoDiscAmt) * (window.TAX_PCT||8) / 100;
    var total = sub + svc - disc - promoDiscAmt + tax;
    if (total < 0) total = 0;

    var el = document.getElementById('billTotal');
    ...
    if (form) {
      ...
      // Also update f_total with promo-applied amount
      setHidden(form, 'f_total', total.toFixed(2));
    }

    // Update debtor after-bill
