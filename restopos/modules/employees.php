...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        // Auto-calc OT from time_out if after 17:00, else use manual
        $otHours = 0;
        if (isset($_POST['ot_manual']) && $_POST['ot_manual'] !== '') {
            $otHours = (float)$_POST['ot_manual'];
        } elseif ($timeOut && in_array($status,['Present','Half Day'])) {
            list($h,$m) = array_map('intval', explode(':', substr($timeOut,0,5)));
            $totalMins  = $h*60+$m;
            $otStartMin = OT_START*60;
            if ($totalMins > $otStartMin) {
                $otHours = round(($totalMins-$otStartMin)/60, 2);
            }
        }
        ...
        foreach ($empIds as $eid) {
            ...
            if (isset($_POST['bulk_ot'][$eid]) && $_POST['bulk_ot'][$eid]!=='') {
                ...
                list($h,$m2) = array_map('intval', explode(':',substr($tout,0,5)));
                $mins = $h*60+$m2;
                if ($mins > OT_START*60) $otHrs = round(($mins-OT_START*60)/60,2);
            }
            if ($tin || $stat !== 'Present') {
...
function calcOTPay(float $basic, float $otH): float { return round(($basic/26/9)*OT_RATE*$otH,2); }
...
<div class="card">
  <div class="flex-between mb-16">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      <span class="badge badge-accent fs-12">⏰ OT auto-calc after 17:00</span>
...
<div class="card">
  ...
  <form method="POST">
    ...
    <div class="alert alert-info fs-12 mb-12">⏰ OT is auto-calculated when Time Out is after 17:00. You can also enter manually.</div>
    <div class="table-wrap">
      <table class="data-table">
...
<div class="card mb-16">
  ...
  <div class="alert alert-info fs-12 mb-16">
    ...
    Formula: Basic ÷ 26 days ÷ 9 hrs × 1.5 × OT hours.
    <br>💡 OT is also auto-calculated from attendance Time Out — if Time Out is after 17:00, OT is calculated automatically.
  </div>
  <form method="POST">
...
<div class="card mb-16">
  ...
  <div class="table-wrap">
    <table class="data-table">
      <thead><tr><th>Date</th><th>Employee</th><th>Position</th><th class="text-right">OT Hours</th><th class="text-right">Hourly Rate</th><th class="text-right">OT Pay (1.5×)</th><th>Notes</th><th>Delete</th></tr></thead>
      <tbody>
  ...
  <div class="modal-box">
    ...
    <form method="POST">
      ...
      <div class="form-row form-row-2 mb-12">
        ...
        <div class="form-group">
          <label class="form-label">Time Out <span class="badge badge-accent" style="font-size:10px">OT auto after 17:00</span></label>
...
function calcModalOT() {
  ...
  let otHours = 0;
  if (tout) {
    const [h, m] = tout.split(':').map(Number);
    const mins = h * 60 + m;
    if (mins > 17 * 60) otHours = Math.round((mins - 17*60) / 60 * 100) / 100;
  }
  document.getElementById('autoOTBadge').textContent = 'Auto: ' + otHours.toFixed(2) + ' hrs';
...
function calcBulkOT(empId) {
  ...
  if (tout) {
    const [h, m] = tout.split(':').map(Number);
    const mins = h*60+m;
    if (mins > 17*60) otH = Math.round((mins-17*60)/60*100)/100;
  }
  const el = document.getElementById('autoOT_' + empId);
  if (el) {
    el.textContent = otH > 0 ? otH.toFixed(2) + ' hrs' : '—';
    el.style.color = otH > 0 ? 'var(--accent)' : 'var(--muted)';
  }
}
